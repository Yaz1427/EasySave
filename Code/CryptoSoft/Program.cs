...
{
    /// <summary>
    /// CryptoSoft - Simple AES-256 file encryption tool for EasySave v3.0
    ...
    /// 
    /// v3.0: Mono-instance - only one CryptoSoft can run at a time.
    /// 
    /// Exit codes:
    ///   0 = Success
    ///   1 = Invalid arguments
    ...
    class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            // v3.0: Mono-instance enforcement using a named Mutex
            using var mutex = new Mutex(false, MutexName);
            bool acquired = false;

            try
            {
                // Wait up to 30 seconds for the mutex (another instance may be finishing)
                acquired = mutex.WaitOne(TimeSpan.FromSeconds(30));
                if (!acquired)
                {
                    Console.Error.WriteLine("Another instance of CryptoSoft is already running. Waiting timed out.");
                    return 4;
                }
            }
